...
class Classifier:
    ...
    def normalize_window_title(self, process_name, window_title):
        """Strip browser suffixes and normalize title to prevent session fragmentation.

        'ChatGPT - Google Chrome' → 'ChatGPT'
        'Inbox (3) - Gmail - Google Chrome' → 'Gmail'
        ...
        # Remove notification-style number suffixes like "Inbox (3)"
        title = re.sub(r'\(\d+\)$', '', title).strip()

        # If after stripping we have "X - Gmail", take the last meaningful segment
        for sep in [" - ", " — ", " | "]:
            if sep in title:
                parts = [p.strip() for p in title.split(sep) if p.strip()]
                # Prefer the rightmost non-noise segment
                if parts:
                    title = parts[-1]

        # Remove remaining notification counts like "(3)"
        title = re.sub(r'\(\d+\)', '', title).strip()
